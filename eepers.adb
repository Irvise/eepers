...
procedure Eepers is
   ...
   Footsteps_Pitches : constant array (Footsteps_Range) of Float :=
     (1.7, 1.6, 1.5, 1.4);
   package Random_Footsteps is new Ada.Numerics.Discrete_Random
     (Result_Subtype => Footsteps_Range);
   ...
   type Palette is
     (COLOR_BACKGROUND, COLOR_FLOOR, COLOR_WALL, COLOR_BARRICADE, COLOR_PLAYER,
      COLOR_DOORKEY, COLOR_BOMB, COLOR_LABEL, COLOR_GUARD, COLOR_MOTHER,
      COLOR_CHECKPOINT, COLOR_EXPLOSION, COLOR_HEALTHBAR, COLOR_EYES,
      COLOR_FATHER);
   ...
   begin
      ...
      while not End_Of_File (F) loop
         ...
         declare
            ...
            function Chop_By
              (Src : in out Unbounded_String; Pattern : String)
            ...
            function Find_Color_By_Key
              (Key : Unbounded_String; Co : out Palette) return Boolean
            is
            begin
               for C in Palette loop
                  if Key = C'Image then
         ...
         begin
            ...
            if Find_Color_By_Key (Key, C) then
               Line                    := Trim (Line, Ada.Strings.Left);
               Palette_HSV (C) (Hue)   :=
                 Byte'Value (To_String (Chop_By (Line, " ")));
               Line                    := Trim (Line, Ada.Strings.Left);
               Palette_HSV (C) (Sat)   :=
                 Byte'Value (To_String (Chop_By (Line, " ")));
               Line                    := Trim (Line, Ada.Strings.Left);
               ...
               Palette_RGB (C)         :=
                 Color_From_HSV
                   (Float (Palette_HSV (C) (Hue)) / 255.0 * 360.0,
                    Float (Palette_HSV (C) (Sat)) / 255.0,
                    Float (Palette_HSV (C) (Value)) / 255.0);
            else
               Put_Line
                 (File_Name & ":" & Line_Number'Image &
                  "WARNING: Unknown Palette Color: """ & To_String (Key) &
                  """");
            end if;
         end;
      end loop;
   ...
   exception
      when E : Name_Error =>
         Put_Line
           ("WARNING: could not load colors from file " & File_Name & ": " &
            Exception_Message (E));
   end Load_Colors;
   ...
   type Cell is
     (Cell_None, Cell_Floor, Cell_Wall, Cell_Barricade, Cell_Door,
      Cell_Explosion);
   ...
   begin
      case C is
         when Cell_None =>
            return Palette_RGB (COLOR_BACKGROUND);
         when Cell_Floor =>
            return Palette_RGB (COLOR_FLOOR);
         when Cell_Wall =>
            return Palette_RGB (COLOR_WALL);
         when Cell_Barricade =>
            return Palette_RGB (COLOR_BARRICADE);
         when Cell_Door =>
            return Palette_RGB (COLOR_DOORKEY);
         when Cell_Explosion =>
            return Palette_RGB (COLOR_EXPLOSION);
      end case;
   end Cell_Colors;

   type Path_Map is array (Positive range <>, Positive range <>) of Integer;
   type Path_Map_Access is access Path_Map;
   procedure Delete_Path_Map is new Ada.Unchecked_Deallocation
     (Path_Map, Path_Map_Access);
   ...
   type Item_Kind is
     (Item_None, Item_Key, Item_Bomb_Refill, Item_Checkpoint, Item_Bomb_Slot);
   type Item is record
      ...
      if Popup.Animation > 0.0 then
         declare
            Font_Size           : constant Float   :=
              Float (Tutorial_Font_Size) * Popup.Animation;
         ...
         begin
            Draw_Text_Ex
              (Tutorial_Font, Popup.Label, Label_Position + (-2.0, 2.0),
               Font_Size, 0.0, Palette_RGB (COLOR_WALL));
            Draw_Text_Ex
              (Tutorial_Font, Popup.Label, Label_Position, Font_Size, 0.0,
               Palette_RGB (COLOR_PLAYER));
         end;
      end if;
   end Draw_Popup;

   type Tutorial_Phase is
     (Tutorial_Move, Tutorial_Place_Bombs, Tutorial_Waiting_For_Sprint,
      Tutorial_Sprint, Tutorial_Done);
   ...
   type Game_State is record
      ...
      Bombs  : Bomb_State_Array;
      Camera : Camera2D :=
        (Offset   => (X => 0.0, Y => 0.0), Target => (X => 0.0, Y => 0.0),
   ...
   begin
      return
        Position.Y in Game.Map'Range (1)
        and then Position.X in Game.Map'Range (2);
   end Within_Map;
   ...
   function Eeper_Can_Stand_Here
     (Game : Game_State; Start : IVector2; Me : Eeper_Index) return Boolean
   is
   ...
   begin
      for X in Start.X .. Start.X + Size.X - 1 loop
         for Y in Start.Y .. Start.Y + Size.Y - 1 loop
            ...
            if Game.Map (Y, X) /= Cell_Floor and
              Game.Map (Y, X) /= Cell_Explosion
            then
               return False;
            end if;
            for Index in Eeper_Index loop
               if not Game.Eepers (Index).Dead and then Index /= Me then
                  ...
                  begin
                     if Inside_Of_Rect (Eeper.Position, Eeper.Size, (X, Y))
                     then
                        return False;
                     end if;
                  end;
      ...
      return True;
   end Eeper_Can_Stand_Here;

   package Queue is new Ada.Containers.Vectors
     (Index_Type => Natural, Element_Type => IVector2);

   procedure Recompute_Path_For_Eeper
     (Game : in out Game_State; Me : Eeper_Index; Steps_Limit : Integer;
      Step_Length_Limit :        Integer; Stop_At_Me : Boolean := True)
   is
      Q     : Queue.Vector;
            ...
            for Dir in Direction loop
               ...
               begin
                  for Limit in 1 .. Step_Length_Limit loop
                     ...
                     Eeper.Path (New_Position.Y, New_Position.X) :=
                       Eeper.Path (Position.Y, Position.X) + 1;
   ...
   procedure Allocate_Item
     (Game : in out Game_State; Position : IVector2; Kind : Item_Kind)
   is
   ...
   type Level_Cell is
     (Level_None, Level_Gnome, Level_Mother, Level_Guard, Level_Floor,
      Level_Wall, Level_Door, Level_Checkpoint, Level_Bomb_Refill,
      Level_Barricade, Level_Key, Level_Player, Level_Father, Level_Bomb_Slot);
   Level_Cell_Color : constant array (Level_Cell) of Color :=
     (Level_None        => Get_Color (16#0000_0000#),
      Level_Gnome       => Get_Color (16#FF96_00FF#),
      Level_Mother      => Get_Color (16#96FF_00FF#),
      Level_Guard       => Get_Color (16#00FF_00FF#),
      Level_Floor       => Get_Color (16#FFFF_FFFF#),
      Level_Wall        => Get_Color (16#0000_00FF#),
      Level_Door        => Get_Color (16#00FF_FFFF#),
      Level_Checkpoint  => Get_Color (16#FF00_FFFF#),
      Level_Bomb_Refill => Get_Color (16#FF00_00FF#),
      Level_Barricade   => Get_Color (16#FF00_96FF#),
      Level_Key         => Get_Color (16#FFFF_00FF#),
      Level_Player      => Get_Color (16#0000_FFFF#),
      Level_Father      => Get_Color (16#265F_DAFF#),
      Level_Bomb_Slot   => Get_Color (16#BC53_53FF#));

   function Cell_By_Color
     (Col : Color; Out_Cel : out Level_Cell) return Boolean
   is
   begin
   ...
   procedure Load_Game_From_Image
     (File_Name : in String; Game : in out Game_State; Update_Player : Boolean;
      Update_Camera :    Boolean)
   is

      Img    : constant Image       := Raylib.Load_Image (File_Name);
      -- TODO FER check that this pointer magic thing holds
      Pixels : constant Color_Array :=
        ((A => 222, others => 0),
         (A => 222, others => 0)); --Raylib.To_Color_Pointer(Img.Data);
   begin
      if Game.Map /= null then
         Delete_Map (Game.Map);
      end if;
                  ...
                  case Cel is
                     ...
                     when Level_Father =>
                        if Update_Camera then
                           Game.Camera.Target :=
                             (To_Vector2 ((Column, Row)) +
                              To_Vector2 ((7, 7)) * 0.5) *
                             Cell_Size;
                           Game.Camera.Offset :=
                             Screen_Size * 0.5 - Cell_Size * 0.5;
                        end if;
                     ...
                     when Level_Floor =>
                        Game.Map (Row, Column) := Cell_Floor;
                     when Level_Wall =>
                        Game.Map (Row, Column) := Cell_Wall;
                     when Level_Door =>
                        Game.Map (Row, Column) := Cell_Door;
                     ...
                     when Level_Player =>
                        ...
                        if Update_Player then
                           ...
                           Game.Player.Eyes_Target   :=
                             Game.Player.Position + Direction_Vector (Up);
                        end if;
                  end case;
               else
                  Game.Map (Row, Column) := Cell_None;
   ...
   begin
      Draw_Circle_V
        (To_Vector2 (Position) * Cell_Size + Cell_Size * 0.5,
         Cell_Size.X * 0.25, Palette_RGB (COLOR_DOORKEY));
   end Draw_Key;
   ...
   begin
      Draw_Text
        (Label, Integer (Position.X), Integer (Position.Y), Label_Height, C);
   end Draw_Number;
               ...
               begin
                  Draw_Rectangle_V
                    (To_Vector2 (Item.Position) * Cell_Size + Cell_Size * 0.5 -
                     ...
                     Checkpoint_Item_Size, Palette_RGB (COLOR_CHECKPOINT));
               end;
   ...
   procedure Flood_Fill
     (Game : in out Game_State; Start : IVector2; Fill : Cell)
   is
      Q          : Queue.Vector;
      Background : Cell;
   begin
      ...
      while not Q.Is_Empty loop
         ...
         begin
            ...
            for Dir in Direction loop
               declare
                  New_Position : constant IVector2 :=
                    Position + Direction_Vector (Dir);
               begin
                  if Within_Map (Game, New_Position)
                    and then Game.Map (New_Position.Y, New_Position.X) =
                      Background
                  then
   ...
   procedure Game_Player_Turn
     (Game : in out Game_State; Step_Direction : Direction)
   is
   begin
      ...
      begin
         Game.Player.Eyes_Target :=
           New_Position + Direction_Vector (Step_Direction);
   ...
   procedure Explode (Game : in out Game_State; Position : in IVector2) is
      ...
      begin
         Line :
         for I in 1 .. EXPLOSION_LENGTH loop
            if not Within_Map (Game, New_Position) then
               return;
            end if;
            ...
            case Game.Map (New_Position.Y, New_Position.X) is
               when Cell_Floor | Cell_Explosion =>
                  ...
                  for Eeper of Game.Eepers loop
                     if not Eeper.Dead
                       and then Inside_Of_Rect
                         (Eeper.Position, Eeper.Size, New_Position)
                     then
                        Eeper.Damaged := True;
                     end if;
                  end loop;
   ...
   procedure Game_Update_Camera (Game : in out Game_State) is
      Camera_Target   : constant Vector2 :=
        To_Vector2 (Game.Player.Position) * Cell_Size;
      Camera_Offset : constant Vector2 := Screen_Size * 0.5 - Cell_Size * 0.5;
      Camera_Velocity : constant Vector2 :=
        (Camera_Target - Game.Camera.Target) * 2.0;
   begin
      ...
      Game.Camera.Zoom   :=
        Float'Max (Screen_Size.X / 1_920.0, Screen_Size.Y / 1_080.0);
   end Game_Update_Camera;

   function Interpolate_Positions
     (IPrev_Position, IPosition : IVector2; T : Float) return Vector2
   is
      Prev_Position : constant Vector2 :=
        To_Vector2 (IPrev_Position) * Cell_Size;
      Curr_Position : constant Vector2 := To_Vector2 (IPosition) * Cell_Size;
   begin
      return Vector2_Lerp (Prev_Position, Curr_Position, 1.0 - T * T);
   end Interpolate_Positions;
   ...
   function Command_Dequeue
     (Q : in out Command_Queue_Record; C : out Command) return Boolean
   is
   begin
      if Q.Size = 0 then
         return False;
            ...
            if not Eeper.Dead then
               ...
               case Eeper.Kind is
                  when Eeper_Father =>
                     ...
                     begin
                        if Inside_Of_Rect
                            (Eeper.Position, Eeper.Size, Game.Player.Position)
                        then
                           Load_Game_From_Image
                             ("assets/map.png", Game, Update_Player => True,
                              Update_Camera                         => False);
                           Game_Save_Checkpoint (Game);
                           Play_Sound (Checkpoint_Sound);
                        ...
                        then
                           ...
                           Eeper.Eyes_Target :=
                             Eeper.Position + (Eeper.Size.X / 2, Eeper.Size.Y);
                           Eeper.Eyes        := Eyes_Closed;
                        end if;
                     end;
                  when Eeper_Guard | Eeper_Mother =>
                     Recompute_Path_For_Eeper
                       (Game, Me, GUARD_STEPS_LIMIT, GUARD_STEP_LENGTH_LIMIT);
                     if Eeper.Path (Eeper.Position.Y, Eeper.Position.X) = 0
                     then
                        Kill_Player (Game);
                        Eeper.Eyes := Eyes_Surprised;
                     elsif Eeper.Path (Eeper.Position.Y, Eeper.Position.X) > 0
                     then
                        if Eeper.Attack_Cooldown <= 0 then
                           declare
                              Current             : constant Integer :=
                                Eeper.Path
                                  (Eeper.Position.Y, Eeper.Position.X);
                              Available_Positions :
                                array
                                  (0 ..
                           ...
                           begin
                              for Dir in Direction loop
                                 ...
                                 begin
                                    while Eeper_Can_Stand_Here
                                        (Game, Position, Me)
                                    loop
                                       Position :=
                                         Position + Direction_Vector (Dir);
                                       if Within_Map (Game, Position)
                                         and then
                                           Eeper.Path
                                 ...
                                 begin
                                    if Within_Map (Game, New_Position)
                                      ...
                                      and then
                                        Eeper.Path
                                          (New_Position.Y, New_Position.X) >
                                        Eeper.Path (Position.Y, Position.X)
                                    then
                                       Available_Positions (Count) :=
                                         New_Position;
                                       Inc (Count);
   ...
   function Repeat (T, Length : Float) return Float is
      function Floorf (A : Float) return Float with
        Import => True, Convention => C, External_Name => "floorf";
   begin
   ...
   procedure Draw_Eyes
     (Start, Size : Vector2; Angle : Float; Prev_Kind, Kind : Eyes_Kind;
      T           : Float)
   is
              ...
              Eyes_Size *
                Vector2_Lerp
                  (Eyes_Meshes (Prev_Kind) (Eye_Index) (Vertex_Index),
                   Eyes_Meshes (Kind) (Eye_Index) (Vertex_Index), 1.0 - T * T);
         end loop;
   ...
   begin
      ...
      Draw_Popup
        (Game.Tutorial.Popup, Screen_Player_Position (Game), Cell_Size);
   end Game_Tutorial;

   procedure Game_Player (Game : in out Game_State) is
      Eyes_Angular_Direction : constant Float :=
        Delta_Angle
          (Game.Player.Eyes_Angle,
           Look_At
             (To_Vector2 (Game.Player.Position) * Cell_Size + Cell_Size * 0.5,
              To_Vector2 (Game.Player.Eyes_Target) * Cell_Size +
              Cell_Size * 0.5));
   begin
      Game.Player.Eyes_Angle :=
        Game.Player.Eyes_Angle +
        Eyes_Angular_Direction * EYES_ANGULAR_VELOCITY * Get_Frame_Time;
      if Game.Player.Dead then
         if Game.Turn_Animation >= 0.0 then
            Draw_Rectangle_V
              (Screen_Player_Position (Game), Cell_Size,
               Palette_RGB (COLOR_PLAYER));
            Draw_Eyes
              (Screen_Player_Position (Game), Cell_Size,
               Game.Player.Eyes_Angle, Game.Player.Prev_Eyes, Game.Player.Eyes,
               Game.Turn_Animation);
         end if;
      ...
      Draw_Eyes
        (Screen_Player_Position (Game), Cell_Size, Game.Player.Eyes_Angle,
         Game.Player.Prev_Eyes, Game.Player.Eyes, Game.Turn_Animation);
                  ...
                  begin
                     ...
                     if Game.Tutorial.Phase = Tutorial_Waiting_For_Sprint then
                        declare
                           ...
                           Delta_Timestamp : constant Long_Float :=
                             Step_Timestamp -
   ...
   begin
      for Bomb of Game.Bombs loop
         if Bomb.Countdown > 0 then
            ...
            Draw_Number
              (Bomb.Position, Bomb.Countdown, Palette_RGB (COLOR_LABEL));
         end if;
      end loop;
   end Game_Bombs;
   ...
   begin
      for Index in 1 .. Game.Player.Keys loop
         declare
            Position : constant Vector2 :=
              (100.0 + Float (Index - 1) * Cell_Size.X, 100.0);
         begin
            Draw_Circle_V
      ...
      for Index in 1 .. Game.Player.Bomb_Slots loop
         declare
            ...
            Position : constant Vector2 :=
              (100.0 + Float (Index - 1) * (Cell_Size.X + Padding), 200.0);
         begin
      ...
      if Game.Player.Dead then
         declare
            ...
            Text_Size : constant Vector2 :=
              Measure_Text_Ex
                (Death_Font, Label, Float (Death_Font_Size), 0.0);
            Position : constant Vector2 := Screen_Size * 0.5 - Text_Size * 0.5;
         begin
            Draw_Text_Ex
              (Death_Font, Label, Position + (-2.0, 2.0),
               Float (Death_Font_Size), 0.0, Palette_RGB (COLOR_WALL));
            Draw_Text_Ex
              (Death_Font, Label, Position, Float (Death_Font_Size), 0.0,
               Palette_RGB (COLOR_PLAYER));
         end;
      end if;
   end Game_Hud;
   ...
   procedure Health_Bar
     (Boundary_Start, Boundary_Size : Vector2; Health : Float)
   is
   ...
   begin
      Draw_Rectangle_V
        (Boundary_Start - (0.0, Health_Padding + Health_Height),
         (Health_Width, Health_Height), Palette_RGB (COLOR_HEALTHBAR));
   end Health_Bar;
   ...
   begin
      ...
      Draw_Number
        (Bubble_Center - (Bubble_Radius, Bubble_Radius),
         (Bubble_Radius, Bubble_Radius) * 2.0, Cooldown, Text_Color);
   end Draw_Cooldown_Timer_Bubble;
      ...
      for Eeper of Game.Eepers loop
         declare
            Position               : constant Vector2 :=
              (if Game.Turn_Animation > 0.0 then
                 Interpolate_Positions
                   (Eeper.Prev_Position, Eeper.Position, Game.Turn_Animation)
            ...
            Eyes_Angular_Direction : constant Float   :=
              Delta_Angle
                (Eeper.Eyes_Angle,
                 Look_At
                   (Position + Size * 0.5,
                     To_Vector2 (Eeper.Eyes_Target) * Cell_Size +
                    Cell_Size * 0.5));
         begin
            if not Eeper.Dead then
               Eeper.Eyes_Angle :=
               ...
               case Eeper.Kind is
                  when Eeper_Father =>
                     ...
                     Draw_Eyes
                       (Position, Size, Eeper.Eyes_Angle, Eeper.Prev_Eyes,
                        Eeper.Eyes, Game.Turn_Animation);
                  when Eeper_Guard | Eeper_Mother =>
                     Draw_Rectangle_V
                       (Position, Size, Palette_RGB (Eeper.Background));
                     ...
                     then
                        Draw_Cooldown_Timer_Bubble
                          (Position, Size, Eeper.Attack_Cooldown,
                           Eeper.Background);
                     elsif Eeper.Path (Eeper.Position.Y, Eeper.Position.X) >= 0
                     then
                        Draw_Cooldown_Timer_Bubble
                  ...
                  when Eeper_Gnome =>
                     ...
                     begin
                        Draw_Rectangle_V
                          (GNOME_START, GNOME_SIZE,
                           Palette_RGB (Eeper.Background));
                        Draw_Eyes
                          (GNOME_START, GNOME_SIZE, Eeper.Eyes_Angle,
                           Eeper.Prev_Eyes, Eeper.Eyes, Game.Turn_Animation);
                     end;
               end case;
            end if;
         end;
...
begin
   ...
   Tutorial_Font    :=
     Load_Font_Ex
       ("assets/fonts/Vollkorn/static/Vollkorn-Regular.ttf",
        Tutorial_Font_Size, 0, 0);
   Gen_Texture_Mipmaps (Tutorial_Font.Texture'Access);
   Death_Font :=
     Load_Font_Ex
       ("assets/fonts/Vollkorn/static/Vollkorn-Regular.ttf", Death_Font_Size,
        0, 0);
   ...
   while not Window_Should_Close loop
      ...
      Holding_Shift :=
        Is_Key_Down (KEY_LEFT_SHIFT) or else Is_Key_Down (KEY_RIGHT_SHIFT);
      if Game.Player.Dead then
         Command_Queue.Size := 0;
      else
      ...
      if Holding_Shift then
         ...
         if Command_Queue.Size /= 0 then
            TURN_DURATION_SECS :=
              BASE_TURN_DURATION_SECS * (1.0 / Float (Command_Queue.Size));
         else
      ...
      if DEVELOPMENT then
         if Is_Key_Pressed (KEY_R) then
            Load_Game_From_Image
              ("assets/map.png", Game, Update_Player => False,
               Update_Camera                         => False);
         end if;
               ...
               if Is_Key_Down (Keys (Up)) then
                  Inc
                    (Palette_HSV (Palette_Editor_Choice)
                       (Palette_Editor_Component));
                  Palette_RGB (Palette_Editor_Choice) :=
      ...
      if Game.Turn_Animation > 0.0 then
         Game.Turn_Animation :=
           (Game.Turn_Animation * TURN_DURATION_SECS - Get_Frame_Time) /
           TURN_DURATION_SECS;
      end if;
      ...
      if DEVELOPMENT then
         if Is_Key_Down (KEY_P) then
            for Row in Game.Map'Range (1) loop
               for Column in Game.Map'Range (2) loop
                  Draw_Number
                    ((Column, Row), Game.Eepers (1).Path (Row, Column),
                     (A => 255, others => 0));
               end loop;
            end loop;
         end if;
      ...
      if Palette_Editor then
         for C in Palette loop
            declare
               ...
               Position     : constant Vector2 :=
                 (200.0,
                  200.0 + Float (Palette'Pos (C)) * Float (Label_Height));
            begin
               Draw_Text
                 (Label, Integer (Position.X), Integer (Position.Y),
                  Label_Height,
               ...
               for Comp in HSV_Comp loop
                  declare
                     Label        : constant String  :=
                       Comp'Image & ": " & Palette_HSV (C) (Comp)'Image;
                     Label_Height : constant Integer := 32;
                     Position     : constant Vector2 :=
                       (X => 600.0 + 200.0 * Float (HSV_Comp'Pos (Comp)),
                        Y =>
                          200.0 +
                          Float (Palette'Pos (C)) * Float (Label_Height));
                  begin
                     Draw_Text
                       (Label, Integer (Position.X), Integer (Position.Y),
                        Label_Height,
                        (if
                           Palette_Editor_Selected and
                           C = Palette_Editor_Choice and
                           Comp = Palette_Editor_Component
                         then (R => 255, A => 255, others => 0)
                         else (others => 255)));
                  end;
               end loop;
